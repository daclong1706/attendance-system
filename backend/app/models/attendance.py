...
class AttendanceSession(db.Model):
    ...
    class_session_id = db.Column(
        db.Integer, db.ForeignKey("class_sections.id"), nullable=False
    )
    date = db.Column(db.Date, nullable=False)
    qr_code_start = db.Column(
        db.String(255), nullable=False
    )  # Đổi TEXT -> VARCHAR để tối ưu lưu trữ
    qr_code_end = db.Column(db.String(255), nullable=False)

    class_session = db.relationship(
        "ClassSection", backref="attendance_sessions"
    )
    __table_args__ = (
        db.UniqueConstraint(
            "class_session_id", "date", name="_class_session_date_uc"
        ),
    )
...
class Attendance(db.Model):
    ...
    attendance_session = db.relationship(
        "AttendanceSession", backref="attendances"
    )

    __table_args__ = (
        db.UniqueConstraint(
            "student_id", "attendance_session_id", name="_student_attendance_uc"
        ),
    )
...
class AttendanceLog(db.Model):
    ...
    attendance_session = db.relationship(
        "AttendanceSession", backref="attendance_logs"
    )
